...
| inputType    | 'phone','more*phone','text' | string   | 'text'    |
...
| 属性               | 说明                                                                                    | 类型                    | 默认值 |
| ------------------ | --------------------------------------------------------------------------------------- | ----------------------- | ------ |
| inputType          | 'phone'                                                                                 | 'more*phone'            | string | null |
| showPhonePrefix    | 是否显示号码前缀选择器(inputType等于phone或more*phone才生效)                            | boolean                 | false  |
...
| morePhoneData      | 当inputType='more*phone',才需要传入，用于渲染MoreSelect组件选项（可查看MoreSelect组件） | Array\<MoreData>        | []     |
| morePhoneTitle     | 当inputType='more*phone',才需要传入，用于渲染MoreSelect组件的title                      | string                  | ''     |
...
<Input morePhoneTitle='请选择' morePhoneData={MoreData} inputType='more*phone' showPhonePrefix placeholder='请输入手机号' id='phone' onChange={( value ) => { console.log( value ) }} />
```

当Input组件声明为`phone`和`more*phone`模式，并开启了号码前缀选择功能，那么onChange的时候的返回体将从原来返回value变成对象，返回当前选中的前缀号码以及input的value。
