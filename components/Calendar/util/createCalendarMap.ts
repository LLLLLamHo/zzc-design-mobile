...
function _dateExtensionMerge(dataInfo: CreateCalendarMap_dataInfo, dateExtension: DateExtension): CalendarMapItemRow {
    ...
    const extensionItem = dateExtension[`${defaultItemInfo.y}/${defaultItemInfo.m + 1 < 10 ? `0${defaultItemInfo.m + 1}` : defaultItemInfo.m + 1}/${defaultItemInfo.d < 10 ? `0${defaultItemInfo.d}` : defaultItemInfo.d}`];
    if (extensionItem) {
